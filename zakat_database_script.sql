...
create table public.hawl_tracking (
  id serial not null,
  utilisateur_id uuid not null,
  date_debut date not null,
  date_echeance date not null,
  date_realisation date null,
  montant_debut numeric(15, 2) null,
  montant_fin numeric(15, 2) null,
  nisab_base character varying(10) null default 'gold'::character varying,
  nisab_valeur numeric(15, 2) null,
  statut character varying(20) null default 'EN_COURS'::character varying,
  interrompu_le date null,
  zakat_annuel_id integer null,
  created_at timestamp with time zone null default now(),
  updated_at timestamp with time zone null default now(),
  constraint hawl_tracking_pkey primary key (id),
  constraint hawl_tracking_utilisateur_id_fkey foreign KEY (utilisateur_id) references profils_utilisateurs (id_utilisateur) on delete CASCADE,
  constraint hawl_tracking_zakat_annuel_id_fkey foreign KEY (zakat_annuel_id) references zakat_annuel (id)
) TABLESPACE pg_default;

create index IF not exists idx_hawl_tracking_user on public.hawl_tracking using btree (utilisateur_id) TABLESPACE pg_default;

create index IF not exists idx_hawl_tracking_statut on public.hawl_tracking using btree (statut) TABLESPACE pg_default;

create trigger update_hawl_tracking_updated_at BEFORE
update on hawl_tracking for EACH row
execute FUNCTION update_updated_at_column ();
...
create table public.profils_utilisateurs (
  ...
  date_anniversaire_zakat date null,
  hawl_actif boolean null default false,
  nisab_base character varying(10) null default 'gold'::character varying,
...
execute FUNCTION update_updated_at_column ();
  ...
  updated_at timestamp without time zone null default CURRENT_TIMESTAMP,
  zakat_annuel_id integer null,
  jours_hawl_restants integer null,
