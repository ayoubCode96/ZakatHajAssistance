  ...
  async getMetalsPrices(currency = "MAD") {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(
          "type_metal, prix_gramme, prix_gramme_24k, prix_gramme_20k, prix_gramme_18k, devise, updated_at",
        )
        .eq("actif", true)
        .eq("devise", currency);
      ...
      if ((!gold || !silver) && currency !== "MAD") {
        ...
        if (!silver)
          silver = fallbackData?.find((m) => m.type_metal === "ARGENT");
      }
      ...
      const metalsData = {
        ...
        gold20k:
          gold.prix_gramme_20k ??
          parseFloat((gold.prix_gramme * (20 / 24)).toFixed(4)),
        gold18k:
          gold.prix_gramme_18k ??
          parseFloat((gold.prix_gramme * 0.75).toFixed(4)),
